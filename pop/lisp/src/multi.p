...
define lconstant Parse_mv_form(name, forms) -> (varlist, nvars, form, forms);
	...
	for sym in_cl_list form do
		conspair(sym, varlist) -> varlist;      ;;; _______varlist is reversed!
		nvars fi_+ 1 -> nvars
	endfor;
	unless ispair(forms) do
...
define nth_value(N) -> n;
	...
	/* _N is the actual number of values on the stack, plus 1 (for the index) */
	check_positive() fi_+ 1 -> n;               /* _n = 1-origin stack index */
	if n fi_< N then
