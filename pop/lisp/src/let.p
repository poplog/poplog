...
define lconstant Parse_let_form(name, forms) -> (varlist, values, forms);
	...
	[% for form in_cl_list list do
		...
		conspair(sym, varlist) -> varlist;      ;;; N.B. _______varlist is reversed
		val;
	endfor %] -> values
enddefine;
...
define parse_declarations(decs) -> (specials, functions, debug, decs);
	...
	[% for form in decs do
		for dec in cdr(form) do             ;;; cdr to skip DECLARE
			...
			back(dec) -> dec;
			if sym == @TYPE then
				;;; car in case ___dec is empty
				car(dec) -> sym
			endif;
			if sym == @SPECIAL then
				append(specials, dec) -> specials
			elseif isfunction_type(sym) then
				/* Note: the type associated with ___sym in Local_var_types
					may be AND'ed with a previous type of ___sym. However,
					for the purposes of lispLOCAL all we need to know is
					that the type of ___sym is some sub-type of FUNCTION.
				*/
