...
procedure(w, d, e, k, overflowchar, padchar, expchar);
	...
	if isreal(arg) then
		...
		if isrational(arg) then
			/* If ___arg is a rational number, then it is coerced to
				be a single float ...
			*/
		...
		/* If _k is 0, then _d digits are printed after the decimal
			point, and a single 0 before the decimal point if the field
			width will allow it. If _k is positive ... ; _k significant
			digits are printed before the decimal point and _d - _k + 1
			digits are printed after. If _k is negative ... ; a single 0
			is printed before the decimal point if the total field width
			will permit it, and after the decimal point are printed first
			-_k 0's and then d + _k significant digits.
		*/
		...
		/* If the parameter _d is omitted ... a value is chosen for
			_d subject to the width constraint _w, the scale factor
			_k, and the constraint that no trailing 0 digits may appear
			in the fraction, except if the fraction to be printed
			is 0, then a single 0 digit should appear after the decimal
			point if permitted by _w.
		*/
		...
		/* Deal with _w parameter if supplied */
		...
		if w then
			...
			/* Print overflow if _w or _e too small */
		...
		/* If ___arg is a complex number or some non-numeric object,
			then it is printed using the format directive ~_wD
		*/
...
--- John Williams, Aug 11 1998
		Now coerces to double float if ___arg is rational.
