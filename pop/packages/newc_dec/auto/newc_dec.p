...
section $-external SIZEOFTYPE => ansi_c_dec newc_dec #: #->
...
define vars new_external_import(sysname, popname, argspec, retspec, errpdr, cbr, sfloat);
lvars   sysname, popname, argspec, retspec, errpdr, cbr, sfloat,
        extpdr_id, nargs, val, Ext_call, spec, bit, fltsingle = 0;
		;
lvars	callpdr,
		coercepdr = false,
	;
	...
	*/
    ...
    /*  Now we're going to do something about single floats.
    */
    if sfloat then
        1 -> bit;
        for spec in_vector argspec do
            if spec = "C_float" then
                bit || fltsingle -> fltsingle;
            endif;
            bit << 1 -> bit;
        endfor;
    endif;
	...
	cons_access(consref(true), {^nargs ^retspec}, true, 1)
        ...
        cons_access(consref(true), {%conspair(nargs,fltsingle),  retspec%}, true, 1)
    endif -> callpdr;
...
define read_kr_procedure(pdr_spec, pdr_name);
lvars	n s variables pdr_spec pdr_name var_types
	...
	;
		...
		new_external_import(C_ref, pdr_name, var_types, pdr_spec, mishap,
                                          false /* cbr */, false /*sfloat*/);
enddefine;

define read_ansi_procedure(pdr_spec, pdr_name);
lvars   name spec  pdr_spec pdr_name var_types ret_spec ret_coerce
    ;
dlocal procedure_header = false;
        ...
        new_external_import(C_ref, pdr_name, var_types, pdr_spec, mishap,
                                          false /* cbr */, true /*sfloat*/);
enddefine;

lvars procedure read_procedure ;
...
define global vars newc_dec;
	lvars item
		;
dlocal read_procedure = read_kr_procedure;
	if pop11_try_nextreaditem("(") then
...
define global vars ansi_c_dec;
    lvars item
        ;
dlocal read_procedure = read_ansi_procedure;
    if pop11_try_nextreaditem("(") then
        parse_tag_attributes();
    endif;
    until pop11_try_nextitem_noautoload("endexternal") do
...
/* --- Revision History ---------------------------------------------------
--- Anthony Worrall 1998
		Added suport for ANSI C declarations see help * ansi_c_dec.p
		(note passing and return of single floats needs to be tested)
