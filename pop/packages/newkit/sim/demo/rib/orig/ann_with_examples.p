...
define eval_unit_input( unit_name, network );
    ...
    vars from_units, from_weights;
    lvars from_unit, from_weight, current_neuron;
    vars bias, unit_type;
...
define eval_network( input_list, network ) -> output_list;
    vars input_list, network, output_list;
    vars unit_name, unit_type, activation;
    vars user_interactor;
...
define ann_print_for_nncf( network );
    vars  net_element;
    lvars network, count_net_element, len_net_element, item, len_item, count_item;
...
define ann_train( network, iterations, restart_training ) -> rmse;
    lvars network, iterations, restart_training;
	vars tss;
    lconstant fname1 = '/tmp/ann_train.tmp.net',
