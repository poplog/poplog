...
max(popmemlim, 1600000) -> popmemlim;
uses objectclass
uses poprulebase
uses prb_extra
uses sim_agent;
...
define :method sim_agent_endrun_trace(agent:sim_agent);
	lvars agent;
	;;;['Data in' ^(sim_name(agent)) ^^(sim_data(agent))]==>
enddefine;
...
define :class rib_agent; isa sim_agent;
    ...
    slot sim_rulesets = rib_rulesets;
...
define :class box_agent; is sim_agent;
    ...
    ;;; Information about rules to be obeyed. Rules defined below.
    slot sim_rulesets = box_rulesets;
...
define straight_line_point( coordinate, unit_vector, origin ) -> point;
    lvars coordinate, unit_vector, origin, point;
...
define crossing_point( P, unit_vectorP, Q, unit_vectorQ ) ->
    ( point, coordinateP, coordinateQ );
    lvars  P, unit_vectorP, Q, unit_vectorQ, point, coordinateP, coordinateQ;
...
define :method eval_sensor(angle, a1:rib_agent, a2:box_agent ) -> value;
    lvars angle, origin = [^(rib_x_pos(a1)) ^(rib_y_pos(a1))];
    lvars alpha, unit_vector, value = 0, count = 0, min_distance;
    lvars distance, pos, line;
    lvars point, alpha_min = angle-22.5, alpha_max = angle+22.5;
...
;;; the RIB and the walls of a box in 8 different directions (0, 45, ..., 315
...
define :method rib_sense_box(a1:sim_agent, a2:sim_agent, dist);
    lvars dist, angle, sensor_value, sensor, old_sensor_value;
...
define :method box_sense_rib(a1:sim_agent, a2:sim_agent, dist);
	...
	fast_for line in box_walls(a1) do
		...
		explode(new_position) -> ( rib_x_pos(a2), rib_y_pos(a2));
	    endif;
	endfor;
    endif
enddefine;
...
define :method draw_me(a:rib_agent);
    rc_jumpto((rib_x_pos(a)-0.005)*400, (rib_y_pos(a)-0.01207)*400);
    0 -> rc_heading;
    repeat 8 times
        rc_draw(0.01 * 400);
	rc_turn(45)
...
define :method print_instance(item:rib_agent);
    printf(
	   ...
	   [% sim_name(item), rib_x_pos(item), rib_y_pos(item),
...
/*
    '------------------------------------------------------' =>
    [running ^(sim_name(agent)) data ^(sim_data(agent))] ==>
*/
/*
...
/*
	...
	['With Data' ^^(sim_data(agent))]==>
*/
enddefine;


define sim_scheduler_pausing_trace(agents, cycle);
	lvars agents, cycle, agent;
...
define ann_ob_avoid_predicate( rule_conditions, condition_pattern ) -> action_pattern;
    lvars rule_conditions, condition_pattern, action_pattern, one_true;
...
define which_min( input_list ) -> (index, min_value);
    lvars input_list, min_value = 1e30, item, i = 1, index = false;
...
define ann_wall_approaching_predicate( rule_conditions, condition_pattern )
    ...
    lvars rule_conditions, condition_pattern, action_pattern;

    ;;; [DEBUG condition_pattern ^condition_pattern] ==>

	;;; ignore velocities
	lvars index, min_value, sensor_values;
		[%
	...
	if min_value >  approach_threshold then
		...
		maplist(
			[1 2 3 4 5 6 7 8],
			procedure(item); lvars item; item == index endprocedure)
	else
		false
...
define ann_wall_following_predicate( rule_conditions, condition_pattern )
    -> action_pattern;
    lvars rule_conditions, condition_pattern, action_pattern, item,
		net_output;
...
define ann_wall_following_map( the_variable, something, something_else );
    lvars the_variable, something, something_else;
...
define :rule novely_detection0 in rib_rules
	[VARS x vy]
...
define :rule novely_detection90 in rib_rules
	[VARS x vx]
...
define :rule novely_detection180 in rib_rules
	[VARS x vy]
...
define :rule novely_detection270 in rib_rules
	[VARS x vx]
...
define :rule obstacle_avoidance in rib_rules
...
define :rule wall_approaching in rib_rules
...
define :rule wall_following in rib_rules
...
define :rule apply_pulse in rib_rules
     [acceleration ?ax ?ay]
;;;	 [POP11 [accelerating ?ax ?ay] => ]
     ...
     ;;; [SAY 'apply_pulse rule fired']
	[SAY pulse ?ax ?ay]
enddefine;


define :rule motion in rib_rules
    [NOT motion rule fired]
    ;
    [POP11 rib_x_pos(sim_myself) + rib_x_velocity(sim_myself) ->
     rib_x_pos(sim_myself);
	 rib_y_pos(sim_myself) + rib_y_velocity(sim_myself) ->
     rib_y_pos(sim_myself)]
    [motion rule fired]
enddefine;

define :rule sense_last in rib_rules
    [NOT acceleration ==]
    ;
    [POP11 vars sensor, value; prb_foreach([new_sense_data range ?sensor ?value],
		       procedure ; value -> rib_old_sensor(sim_myself)(sensor);
		       endprocedure )]
    [NOT new_sense_data ==]
    [NOT = rule fired]
    [STOP]
...
define rib_interactor(input_list, network) -> output_list;
    lvars input_list, network, output_list, sensors = tl(tl(input_list));
...
define rib_train(ncurves);
    lvars rib, velocity, ncurves, curve, polyline;
...
define sample(lines, step_len ) -> points;
    lvars lines, points, start_point, end_point, coordinate, line_len,
	  point, dx, dy;
...
define smoothcurve(pointlist,alpha) -> smoothed_curve;
    lvars pointlist, point, smoothed_point;
...
define get_curve(box, redraw ) -> list_of_lines;
    lvars box, redraw, example, list_of_lines;
...
define eval_accelerations( trajectory, distance ) -> accelerations;
    lvars trajectory, accelerations;
