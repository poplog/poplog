...
/* In the code below
	...
	lvar _____fsign refers to string holding a sign, i.e. '+', '-', or ''

	lvar _f refers to a positive floating point number
...
define Upper_expt(f);
	lvars l;
	/* Calculate exponent where (10 ** (n - 1)) <= _f < (10 ** n) */
	if f = 0 then
		0
	else
		log10(f) -> l;
...
define Pr_float(fsign, f, pop_pr_places, leading_0, trailing_0);
	...
	cuch_string(fsign);         ;;; _____fsign is '+', '-', or ''
