...
procedure(w, d, k, overflowchar, padchar);
	...
	if isreal(arg) then
		...
		if w then
			...
			if d then
				...
				/* Leading 0's are not permitted, except that a single
					...
					this 0 is not printed if _w = _d + 1
				*/
				...
				/* If _d is omitted then there is no constraint on the
					number of digits to appear after the decimal point.
					A value for _d is chosen so that as many digits as
					possible are printed subject to the width constraint
					_w and the constraint that no trailing 0's may
				...
				*/
				...
				/* JW - must know how many significant digits after decimal
					point in ___arg to know how many _______padchar's needed
				*/
		...
		elseif d then
			...
			/* If the _w parameter is omitted, ... a value for _w is
				chosen in such a way that no leading pad characters
				need to be printed and exactly _d characters will
			...
			*/
			...
			/* If both _w and _d are omitted, the effect is to
