/* --- Copyright University of Sussex 1994. All rights reserved. ----------
 > File:            $poplocal/local/vision/lib/in_array.p
 > Purpose:         Iteration over data in arrays
 > Author:          David S Young, Jan 25 1994 (see revisions)
...
-----------------------------------------------------------------------
IMPORTANT: THIS LOCAL VERSION IN THE VISION COLLECTION WILL BE
SUPERSEDED BY THE MAIN SYSTEM VERSION AT SOME POINT.  ONCE THE
NORMAL POPLOG SYSTEM INVOKED BY POPLOGIN CONTAINS IN_ARRAY, THE
FOLLOWING FILES SHOULD BE DELETED:

    $poplocal/local/vision/lib/in_array.p
    $poplocal/local/vision/lib/in_region.p
    $poplocal/local/vision/help/in_array
    $poplocal/local/vision/help/in_region
-----------------------------------------------------------------------
...
rectangular in 2-D and the equivalent in other dimensions.
 ...
 -- Run-time procedures to check arrays and get region
 -- Run-time procedures to check for 1-D case
...
define lconstant check_region(narrs, region) -> ndim;
...
define lconstant min_region(array, narrs) -> (ndim, region);
...
define lconstant arrayvec_index(/* coords, */ arr) /* -> index */;
...
define lconstant arrays_1d(narrs, region, startpts) -> rsize;
...
define lconstant getcoords(region, coords, mincoords, maxcoords, ndim);
...
define lconstant plantVLOOPCODE(
    ...
    sysPUSH(REGION);
    sysPUSHQ(startpts);
    sysCALLQ(arrays_1d);        ;;; arrays_1d(narrs, region, startpts)
    sysPUSHS(true);
    sysPOP(RSIZE);              ;;; ->> RSIZE
...
--- David S Young, Feb  1 1994
        Added note at top about when to delete this file.
--- David S Young, Jan 27 1994
        Added in_region.
        Uses array_subscrp instead of arrayvector.datakey.class_fast_subscr
 */
