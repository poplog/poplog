...
define Do_span_insert(instring, _size, _dattr, _scol);
	...
	if issimple(instring) or ispair(instring) then
		;;; ________instring is a single character (and ______size is 1)
		instring -> _dchar;
		...
		;;; ________instring is a (d)string -- add _______dattr to each char
		...
		if vednocharinsert and not(vedstatic) then
			...
			elseif vedstatic then
				...
				if vedscreenwrap then
					;;; chop ______size chars from the end of line to
					;;; prevent wrap-around
