...
ved_regexp_compile(________string_1, ________string_2,                       [_________procedure]
					___________ignore_case, _____embed) -> ________search_p
ved_regexp_try_compile(________string_1, ________string_2,                   [_________procedure]
						___________ignore_case, _____embed) -> ________search_p
		Constructs a search procedure  for use with  ved_regexp_search ,
		or ved_regexp_substitute  . ________string_1  specifies what  to  search
		for. ________string_2 is a (d)string specifying what to replace it with,
		or false if there isn't  a substitution string (ie. if  ________search_p
		is only going to be used with ved_regexp_search ).

		The strings  are  itemised  first (see  ___REF * _______ITEMISE)  and  may
		therefore contain  backslash  sequences  as  recognised  by  the
		Poplog itemiser (eg. \n, \s, \t, etc.).

		________string_1 may  contain  wildcard  search  patterns  (see  'Search
		Pattern Elements' above), and ________string_2 may contain  substitution
		patterns (See 'Substitution Pattern Elements' above).

		If ___________ignore_case is  false ,  then the search  is case  sensitive,
		otherwise the search  is case-insensitive. If  _____embed is  false ,
...
ved_regexp_search(________search_p, _________constrain, ____back, ____wrap, _____start, _n) [_________procedure]
				-> (__________first_line, ____________first_column, _________last_line, ___________last_column)
		Searches forwards or  backwards within the  region specified  by
		_________constrain (and  possibly  wrapping over  the  end of  a  region)
		looking for  a  match  using  ________search_p  (see  ved_regexp_compile
		above).

		_________constrain dictates  the region  in the  vedbuffer to  which  the
		search is constrained. _________constrain can be any one of the following
		words:
		...
		_________constrain can also be a vector of vedline/vedcolumn positions of
		the form:

			{% START_LINE, START_COLUMN, END_LINE, END_COLUMN %}

		If ____back is false , searching  goes towards the end of the  file,
		otherwise it  goes  towards  the beginning  of  the  file  (i.e.
		backwards).

		_____start is either false , true , or 0, as follows:
		...
		If ____wrap is true ,  and the end of  the search region is  reached
		...
		If _n is an integer, the _n'th match is searched for.

		If no match  is found, __________first_line,  ____________first_column, _________last_line  and
		___________last_column  will   be  false   .   If  the   search   succeeds,
		ved_regexp_search   returns   four   VED   buffer    coordinates
		__________first_line, ____________first_column, _________last_line, ___________last_column where

			vedjumpto(__________first_line, ____________first_column)

		will place the cursor on the start of the match, and

			vedjumpto(_________last_line, ___________last_column + 1)

		will place the cursor one character after then end of the match.

		Note that ___________last_column will be one less than ____________first_column if  the
		...
		Similarly, ___________last_column will  be zero  if all  of the  characters
		including the line break on previous line to _________last_line are  part
...
ved_regexp_jumpto(______line_1, ________column_1, ______line_2, ________column_2, ____bool)  [_________procedure]
		...
		In addition, if XVed is running and ____bool is true , the  matching
		text will be selected and placed on the clipboard.


ved_regexp_substitute(________search_p, _________constrain, ____back, ____wrap, _____start, [_________procedure]
					  _n, _____________once_per_line, _________ask_first, ______silent)
		...
		_n is an integer indicating  the maximum number of  substitutions
		to perform. If it is false  , as many substitutions as  possible
		are done.

		If _____________once_per_line  is  false  ,  every  match  with  ________search_p  is
		replaced, otherwise only  the first match  (forwards search)  or
		last match (backwards  search) on  each line  is replaced.  Note
		that if  _____________once_per_line is  true  , the  search starts  from  the
		start (forward search) or end  (backward search) of the  initial
		line.

		If _________ask_first  is  false  ,  all possible  substitutions  in  the
		region  are  done.  If  it  is  true  ,  the  user  is  prompted
		interactively before each substitution.

		If ______silent is false  , the user  is kept informed as to how  many
...
enddefine; /* ved_regexp_search */
...
define ved_regexp_substitute(search_p, constrain, backward, wrap, fromhere,
	...
	if fromhere == true then
		...
		;;; we really do want to start ____here, not one step beyond ...
		0 -> fromhere;
	endif;
...
enddefine; /* ved_regexp_substitute */
...
enddefine; /* Prepare_search */
...
enddefine; /* Split_search_string */
...
enddefine; /* ved_search_or_substitute */
...
--- John Gibson, Apr  3 1996
		Made local pop_exception_final in Itemise_string test fro appropriate
		id-string.
--- John Gibson, Mar 11 1996
		Made ved_regexp_substitute locally set vvedpromptchar false.
--- John Gibson, Feb  6 1996
		Itemise_string uses local pop_exception_final instead of
		pr*mishap
...
--- Robert John Duncan, Jul 27 1995
		...
		name and argument when ____mode == "undef", i.e. for commands such as
...
--- Robert John Duncan, Jun  7 1995
		...
		from the cursor position (________fromhere = 0) and finishes with the cursor
		on the last match or substitution, rather than always on the last
		substitution (i.e. ________saveline/_______savecol are set for each successful match
...
--- Robert John Duncan, Jun  6 1995
		...
		was caused by Do_substitution resetting _________firstline and ________firstcol to
...
--- John Gibson, Jan 17 1994
		Fixed ved_search_or_substitute to set _____delim correctly when redoing
